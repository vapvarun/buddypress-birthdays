...
|---|---|---|---|
| 2026-07-10 | feature | GDPR per-member birthday opt-out (Basecamp card 9749778137). New `BP_Birthdays_Helpers::is_user_opted_out()` (meta `bb_birthday_hidden`, filter `bb_birthday_user_opted_out`) gates BOTH central data paths: widget/shortcode `bbirthdays_get_array()` and cron `get_todays_birthdays()` (covers activity, BP notifications, emails, admin summary). Member self-service checkbox on BuddyPress Settings > General (`bp_core_general_settings_before_submit`/`_after_save`). Verified: opt-out hides member from widget; save/delete round-trip persists. | `includes/class-bp-birthdays-helpers.php`, `assets/inc/buddypress-birthdays-widget.php`, `includes/class-bp-birthdays-notifications.php`, `core-init.php` |
| 2026-07-10 | bug-fix | Card 9749776914 (blank AJAX on invalid wish) was ALREADY-OK (fall-through already sent `wp_send_json_error`); refined `mark_wished` to return explicit `Authentication required` vs `Invalid request: missing user_id` instead of one misleading message. | `core-init.php` |
| 2026-07-10 | release-prep | readme + main header: `Tested up to: 7.0`, added `Requires Plugins: buddypress`; 2.5.0 changelog gains GDPR opt-out (New) + WP 7.0 (Compat). RTL: 4 physical props in `bb-core.css` → logical (`inset-inline-end`, `margin-inline-start`); `.min` synced. | `readme.txt`, `buddypress-birthdays.php`, `assets/css/bb-core.css`, `assets/css/bb-core.min.css` |
